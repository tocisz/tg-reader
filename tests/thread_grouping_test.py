...
class TestThreadGrouping(unittest.TestCase):
    def test_grouping(self):
        ...
        # Thread for root A should include 1,2,4,3
        # 4 follows 2, because it is a reply to 2
        ...
        self.assertEqual([msg['id'] for msg in threads[1]], [1,2,4,3])
    ...
    def test_depth_property(self):
        ...
        threads = group_threads(messages)
        # Check depth property for thread A
        a_thread = threads[1]
        expected_depths = {1: 0, 2: 1, 4: 2, 3: 1}
        for msg in a_thread:
            self.assertIn('depth', msg)
            self.assertEqual(msg['depth'], expected_depths[msg['id']])
        # Check depth property for thread B
        b_thread = threads[5]
        expected_depths_b = {5: 0, 6: 1}
        for msg in b_thread:
            self.assertIn('depth', msg)
            self.assertEqual(msg['depth'], expected_depths_b[msg['id']])
